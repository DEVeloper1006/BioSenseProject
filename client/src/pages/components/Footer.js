...
const Footer = () => {
  function sendEmail() {
    ...
    if (!message) alert("Please enter feedback before sending");
      ...
      emailjs.send("service_uod7oc4", "template_gj4cqp7", { message }).then(
        () => {
    ...
    <footer className="footer items-center p-4 bg-neutral text-neutral-content">
      ...
      <div className="grid-flow-col gap-4 md:place-self-center md:justify-self-end flex-wrap">
        <input
          ...
          className="w-full max-w-xs outline-none p-3 rounded-xl bg-neutral border-2 xs: text-md"
        />
        <button
          className="btn btn-accent hover:scale-105 rounded-xl"
          onClick={sendEmail}
        >
          Send Feedback
        </button>
